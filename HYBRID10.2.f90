...
! write (*, *) 'Writing to ', trim (file_name)
 ...
 CALL CHECK (NF90_PUT_VAR (ncid, lon_varid, REAL (lon)))
 CALL CHECK (NF90_PUT_VAR (ncid, lat_varid, REAL (lat)))
 CALL CHECK (NF90_PUT_VAR (ncid,     varidW, REAL (soilW_grid)))
 CALL CHECK (NF90_PUT_VAR (ncid,     varidB, REAL (B_grid)))
 CALL CHECK (NF90_PUT_VAR (ncid,     varidSOM, REAL (SOM_grid)))
 CALL CHECK (NF90_close (ncid))
